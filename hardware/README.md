...
* **Analiza Aktywności (Bajt 3):** Głównym zarządcą sieci jest moduł Gateway (ID `0x42B`). W Bajcie 3 swojej ramki zgłasza on zapotrzebowanie różnych systemów na zasilanie (np. `0x40` silnik, `0x02` moduł komfortu). Nasz układ (Infotainment) ma przypisaną wartość `0x04`. Kod wykonuje operację bitową `(rxBuf[3] & 0xFB) != 0`, co pozwala zignorować nasz własny status i sprawdzić, czy jakikolwiek inny moduł w aucie żąda utrzymania komunikacji.
* **Aktywne Podtrzymanie (Ring):** Dopóki warunek aktywności jest spełniony, Arduino aktywnie uczestniczy w ringu OSEK. Na każde odpytanie z Gatewaya (Bajt 0 = `0x0B`), Arduino odpowiada własną ramką `0x40B`. Ustawia w niej opkod `0x02` (Ring) lub `0x01` (Alive - powrót po błędzie). Jednocześnie uruchamiany jest asynchroniczny timer, który co 150 ms wysyła ramkę `0x661` (status włączonego radia).
* **Procedura Uśpienia (Sleep Mode):** Gdy znikają powody aktywności (np. wyłączono zapłon i zamknięto drzwi), Bajt 3 z Gatewaya przyjmuje wartość `0x00`. Warunek aktywności przestaje być spełniony. Arduino natychmiast przestaje wysyłać ramki `0x40B` oraz `0x661`. Całkowita cisza ze strony układu pozwala modułowi Gateway na wysłanie oficjalnej flagi uśpienia **SLEEP_INDICATION** (`0x10` w Bajcie 1) i sprzętowe wygaszenie magistrali.
...
* **Logika OSEK:** Detekcja ramki `0x42B`. Ocena stanu uśpienia (Bajt 1) oraz obliczenie warunku aktywności (Bajt 3). Ewentualne wysłanie ramki `0x40B`.
...
* **Pompka Radia:** Jeśli warunek aktywności jest spełniony, timer wysyła ramkę `0x661` co 150 ms.
